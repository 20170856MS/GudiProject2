...
btnRoom.addEventListener("click",function(){
	...
	.done(function(data){
		$(data).each(function(){
            var arrSeTime1 = this.seTime.split(',')
            console.log(arrSeTime1 + " ");
            arrSeTime.push(arrSeTime1);
            });
            console.log("배열에 다 넣음 : "+arrSeTime)
            if(arrSeTime.length != 0){
                for(let i=0; i<arrSeTime.length;i++){
                    for(let j = 0; j<arrSeTime[i].length; j++){
                        console.log(arrSeTime[i][j])
                        for(let k=0; k<timeBtn.length; k++){
                            if(timeBtn[k].value == arrSeTime[i][j]){
                                const td_contents = document.createAttribute("disabled")
                                
                                timeBtn[k].setAttributeNode(td_contents)
                            }
                        }
                    }
